...
def create_from_path_shapefile(received_table: ReceivedTable) -> pl.DataFrame:
    if not isinstance(received_table.table_settings, InputShapefileTable):
...
def create_from_path_geoparquet(received_table: ReceivedTable) -> pl.DataFrame:
    if not isinstance(received_table.table_settings, InputGeoParquetTable):
...
def create_from_path_geojson(received_table: ReceivedTable) -> pl.DataFrame:
    if not isinstance(received_table.table_settings, InputGeoJsonTable):
